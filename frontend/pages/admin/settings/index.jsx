...
export default function Settings() {
    ...
    const handleToggleActive = (id) => {
        setGateways(prev => prev.map(g => ({ ...g, is_active: g.id === id })));
    };
                            ...
                            <div key={gateway.id} className={`bg-white rounded-xl border p-6 transition-all ${gateway.is_active ? 'border-copper ring-1 ring-copper shadow-sm' : 'border-gray-200 opacity-90'}`}>
                                ...
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <div className="flex gap-2">
                                        <button onClick={() => handleToggleActive(gateway.id)} className={`px-3 py-1 text-xs rounded-full ${gateway.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100'}`}>
